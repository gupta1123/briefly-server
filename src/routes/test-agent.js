...
import { buildQueryPlan, extractFilters } from '../lib/query-planner.js';
...
import { routeWithTools } from '../agents/router2.js';
import { executePlan } from '../agents/planner.js';
...
export function registerTestAgentRoutes(app) {
  ...
  app.post('/orgs/:orgId/chat/query', {
    ...
    try {
      ...
      // Toggle: tool-aware router + planner (TEST_AGENT_USE_TOOL_PLANNER=true). Run before degraded check.
      let useToolPlanner = String(process.env.TEST_AGENT_USE_TOOL_PLANNER || 'false').toLowerCase() === 'true';
      if (typeof useToolPlannerFlag === 'boolean') useToolPlanner = useToolPlannerFlag;
      // Global override to force orchestrator-only mode
      const forceOrchestrator = String(process.env.TEST_AGENT_FORCE_ORCHESTRATOR || 'false').toLowerCase() === 'true';
      if (forceOrchestrator) useToolPlanner = false;
      if (useToolPlanner) {
        try {
          // Log that we are using the tool planner path
          try { req.log?.info({ orgId, userId, scope: context?.scope }, 'tool-planner: starting'); } catch {}
          const decision = await routeWithTools({ db, orgId, question, conversation, memory: userMemory, context });
          try { req.log?.info({ orgId, userId, intent: decision?.intent, primaryTool: decision?.primaryTool, confidence: decision?.confidence }, 'tool-planner: decision'); } catch {}
          const resultPlan = await executePlan({ app, db, orgId, question, conversation, memory: userMemory, context, decision, allowedDocIds: Array.from(allowedDocIds), options: { strictCitations: !!strictCitations, filters } });
          try { req.log?.info({ orgId, userId, citations: Array.isArray(resultPlan?.citations) ? resultPlan.citations.length : 0 }, 'tool-planner: executed'); } catch {}
          // Normalize citations within allowed set
          let citations = Array.isArray(resultPlan?.citations) ? resultPlan.citations : [];
          if (allowedDocIds.size) citations = citations.filter(c => c && c.docId && allowedDocIds.has(c.docId));
          const considered = resultPlan?.considered || { docIds: [], strategy: context?.scope || 'org' };
          if (allowedDocIds.size && Array.isArray(considered.docIds)) considered.docIds = considered.docIds.filter(id => allowedDocIds.has(id));

          const answerText = String(resultPlan?.answer || '').trim();
          const genericInsufficient = /I don't have enough information/i.test(answerText);
          const importantIntent = decision.intent === 'DocCount' || decision.intent === 'ListDocs' || decision.intent === 'Linked';

          // If planner result looks weak (generic insufficient OR no-citation very short), fall through to full orchestrator pipeline
          if (!importantIntent && (genericInsufficient || (citations.length === 0 && answerText.length < 12))) {
            try { req.log?.info({ orgId, userId }, 'tool-planner: weak result; falling back to orchestrator'); } catch {}
            // Do not return; continue below into retrieval + orchestrator path
          } else {
            return reply.send({
              answer: answerText || 'No answer.',
              citations,
              agent: { type: decision.intent, name: decision.primaryTool || 'Planner', confidence: decision.confidence || 0.6 },
              considered,
              confidence: typeof resultPlan?.confidence === 'number' ? resultPlan.confidence : undefined,
              coverage: typeof resultPlan?.coverage === 'number' ? resultPlan.coverage : undefined,
              executionTrace: [],
            });
          }
        } catch (plannerErr) {
          try { req.log?.warn?.(plannerErr, 'tool-planner: failed; continuing'); } catch {}
        }
      }
  ...
  app.post('/orgs/:orgId/chat/ask-rest', { 
    ...
    try {
      ...
      console.log('🤖 Processing with coordinated multi-agent approach');
      
      let agentResponse;
      try {
        agentResponse = await agentOrchestrator.processWithCoordination(
          db, 
          question, 
          documents, 
          conversation,
        ...
        console.error('Agent orchestration failed:', agentError);
        
        // Fallback to single agent processing
        try {
          if (agentOrchestrator?.enhancedOrchestrator?.processQuestion) {
            agentResponse = await agentOrchestrator.enhancedOrchestrator.processQuestion(
              db, 
              question, 
              documents, 
              conversation,
              routingResult?.agentType || null
            );
          } else {
            agentResponse = await agentOrchestrator.processQuestion(
              db, 
              question, 
              documents, 
              conversation,
          ...
          console.error('Fallback agent processing failed:', fallbackError);
          
          // Final fallback to basic response
          agentResponse = {
            answer: 'I encountered an error while processing your request. This might be due to rate limits or service availability. Please try rephrasing your question or try again later.',
            confidence: 0.1,
            citations: [],
            agentType: 'error',
...
async function processWithEnhancedAgent(
  ...
  console.log('🤖 Processing with agent:', routingResult.agentName);
  
  // Based on the agent type, route to appropriate processing logic
  switch (routingResult.agentType) {
    case 'metadata':
      return await processMetadataQuery(db, orgId, question, conversation, userMemory, routingResult);
    case 'content':
      return await processContentQuery(db, orgId, userId, question, conversation, userMemory, routingResult);
    case 'financial':
      return await processFinancialQuery(db, orgId, question, conversation, userMemory, routingResult);
    case 'legal':
      return await processLegalQuery(db, orgId, question, conversation, userMemory, routingResult);
    case 'resume':
      return await processResumeQuery(db, orgId, question, conversation, userMemory, routingResult);
    case 'casual':
      return await processCasualQuery(question, conversation, routingResult);
    default:
...
async function processMetadataQuery(db, orgId, question, conversation, userMemory, routingResult) {
...
async function processFinancialQuery(db, orgId, question, conversation, userMemory, routingResult) {
...
async function processLegalQuery(db, orgId, question, conversation, userMemory, routingResult) {
  console.log('⚖️ Processing legal query');
...
async function processResumeQuery(db, orgId, question, conversation, userMemory, routingResult) {
  console.log('📄 Processing resume query');
