...
/**
 * Test script for enhanced agent routes with authentication simulation
 * Run with: node test-agent-with-auth.js
 */

const BASE_URL = process.env.API_BASE_URL || 'http://localhost:8787';

async function testAgentWithAuth() {
  console.log('🧪 Testing Enhanced Agent Routes with Auth Simulation\n');
  ...
  // Test 2: Test the new agent endpoint with proper structure
  console.log('\n2. Testing new agent v2 endpoint structure...');
  try {
    ...
    console.log('✅ New agent v2 endpoint OPTIONS response:', optionsResponse.status);
    
    // Get allowed methods from headers if available
  ...
  console.log('  4. Test the enhanced agent functionality');
